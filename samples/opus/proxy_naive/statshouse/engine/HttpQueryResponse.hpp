...
public:
    ...
    bool verify() const noexcept
    {
        ...
        if (const auto& return_code = get_return_code(); return_code && !return_code->verify())
            return false;
    ...
    std::optional<Int> get_return_code() const noexcept
    {
        return PROXY_1_RETURN_CODE_OFFSET == NO_VALUE ? std::optional<Int>() : Int {m_proxy_1, PROXY_1_RETURN_CODE_OFFSET};
    }
    ...
    static HttpQueryResponse fetch(InputStream& stream)
    {
        ...
        offset_t PROXY_1_SIZEOF = (IS_SET(fields_mask, 0) ? Int::SIZEOF : 0);
...
private:
        ...
        , m_additional_headers(std::move(additional_headers))
    {
        ...
        if (IS_SET(m_fields_mask, 0)) {
            PROXY_1_RETURN_CODE_OFFSET = proxy_1_pos;
            proxy_1_pos += Int::SIZEOF;
        }
    }

    Nat m_fields_mask;
    offset_t PROXY_1_SIZEOF;
    offset_t PROXY_1_RETURN_CODE_OFFSET = NO_VALUE;
    Proxy m_proxy_1;

...
public:
    ...
    bool verify() const noexcept
    {
        ...
        if (const auto& counter_eq_1 = get_counter_eq_1(); counter_eq_1 && !counter_eq_1->verify())
            return false;

        if (const auto& value_set = get_value_set(); value_set && !value_set->verify())
            return false;

        if (const auto& value_min = get_value_min(); value_min && !value_min->verify())
            return false;

        if (const auto& value_max = get_value_max(); value_max && !value_max->verify())
            return false;

        if (const auto& value_sum = get_value_sum(); value_sum && !value_sum->verify())
            return false;

        if (const auto& value_sum_square = get_value_sum_square(); value_sum_square && !value_sum_square->verify())
            return false;

        if (const auto& max_host_tag = get_max_host_tag(); max_host_tag && !max_host_tag->verify())
            return false;

        if (const auto& min_host_tag = get_min_host_tag(); min_host_tag && !min_host_tag->verify())
            return false;

        if (const auto& max_counter_host_tag = get_max_counter_host_tag(); max_counter_host_tag && !max_counter_host_tag->verify())
            return false;
    ...
    std::optional<true_t> get_counter_eq_1() const noexcept
    {
        return PROXY_1_COUNTER_EQ_1_OFFSET == NO_VALUE ? std::optional<true_t>() : true_t {m_proxy_1, PROXY_1_COUNTER_EQ_1_OFFSET};
    }

    std::optional<true_t> get_value_set() const noexcept
    {
        return PROXY_1_VALUE_SET_OFFSET == NO_VALUE ? std::optional<true_t>() : true_t {m_proxy_1, PROXY_1_VALUE_SET_OFFSET};
    }

    std::optional<Double> get_value_min() const noexcept
    {
        return PROXY_1_VALUE_MIN_OFFSET == NO_VALUE ? std::optional<Double>() : Double {m_proxy_1, PROXY_1_VALUE_MIN_OFFSET};
    }

    std::optional<Double> get_value_max() const noexcept
    {
        return PROXY_1_VALUE_MAX_OFFSET == NO_VALUE ? std::optional<Double>() : Double {m_proxy_1, PROXY_1_VALUE_MAX_OFFSET};
    }

    std::optional<Double> get_value_sum() const noexcept
    {
        return PROXY_1_VALUE_SUM_OFFSET == NO_VALUE ? std::optional<Double>() : Double {m_proxy_1, PROXY_1_VALUE_SUM_OFFSET};
    }

    std::optional<Double> get_value_sum_square() const noexcept
    {
        return PROXY_1_VALUE_SQUARE_OFFSET == NO_VALUE ? std::optional<Double>() : Double {m_proxy_1, PROXY_1_VALUE_SQUARE_OFFSET};
    }
    ...
    std::optional<Int> get_max_host_tag() const noexcept
    {
        return PROXY_2_MAX_HOST_TAG_OFFSET == NO_VALUE ? std::optional<Int>() : Int {m_proxy_2, PROXY_2_MAX_HOST_TAG_OFFSET};
    }

    std::optional<Int> get_min_host_tag() const noexcept
    {
        return PROXY_2_MIN_HOST_TAG_OFFSET == NO_VALUE ? std::optional<Int>() : Int {m_proxy_2, PROXY_2_MIN_HOST_TAG_OFFSET};
    }

    std::optional<Int> get_max_counter_host_tag() const noexcept
    {
        return PROXY_2_MAX_COUNTER_HOST_TAG_OFFSET == NO_VALUE ? std::optional<Int>() : Int {m_proxy_2, PROXY_2_MAX_COUNTER_HOST_TAG_OFFSET};
    }
    ...
    static MultiValue_BASE fetch(InputStream& stream, Nat fields_mask)
    {
        ...
        offset_t PROXY_1_SIZEOF = (IS_SET(fields_mask, 0) ? Double::SIZEOF : 0)
                                  ...
                                  + (IS_SET(fields_mask, 2) ? true_t::SIZEOF : 0)
                                  + (IS_SET(fields_mask, 3) ? Double::SIZEOF : 0)
                                  + (IS_SET(fields_mask, 4) ? Double::SIZEOF : 0)
                                  + (IS_SET(fields_mask, 4) ? Double::SIZEOF : 0)
                                  + (IS_SET(fields_mask, 4) ? Double::SIZEOF : 0);
        offset_t proxy_1_pos = 0;
        offset_t PROXY_1_COUNTER_OFFSET = NO_VALUE;
        offset_t PROXY_1_COUNTER_EQ_1_OFFSET = NO_VALUE;
        offset_t PROXY_1_VALUE_SET_OFFSET = NO_VALUE;
        offset_t PROXY_1_VALUE_MIN_OFFSET = NO_VALUE;
        offset_t PROXY_1_VALUE_MAX_OFFSET = NO_VALUE;
        offset_t PROXY_1_VALUE_SUM_OFFSET = NO_VALUE;
        offset_t PROXY_1_VALUE_SQUARE_OFFSET = NO_VALUE;
        if (IS_SET(fields_mask, 0)) {
            PROXY_1_COUNTER_OFFSET = proxy_1_pos;
            proxy_1_pos += Double::SIZEOF;
        }
        if (IS_SET(fields_mask, 1)) {
            PROXY_1_COUNTER_EQ_1_OFFSET = proxy_1_pos;
            proxy_1_pos += true_t::SIZEOF;
        }
        if (IS_SET(fields_mask, 2)) {
            PROXY_1_VALUE_SET_OFFSET = proxy_1_pos;
            proxy_1_pos += true_t::SIZEOF;
        }
        if (IS_SET(fields_mask, 3)) {
            PROXY_1_VALUE_MIN_OFFSET = proxy_1_pos;
            proxy_1_pos += Double::SIZEOF;
        }
        if (IS_SET(fields_mask, 4)) {
            PROXY_1_VALUE_MAX_OFFSET = proxy_1_pos;
            proxy_1_pos += Double::SIZEOF;
        }
        if (IS_SET(fields_mask, 4)) {
            PROXY_1_VALUE_SUM_OFFSET = proxy_1_pos;
            proxy_1_pos += Double::SIZEOF;
        }
        if (IS_SET(fields_mask, 4)) {
            PROXY_1_VALUE_SQUARE_OFFSET = proxy_1_pos;
            proxy_1_pos += Double::SIZEOF;
        }
        ...
        offset_t PROXY_2_SIZEOF = (IS_SET(fields_mask, 7) ? Int::SIZEOF : 0)
                                  + (IS_SET(fields_mask, 8) ? Int::SIZEOF : 0)
                                  + (IS_SET(fields_mask, 9) ? Int::SIZEOF : 0);
        offset_t proxy_2_pos = 0;
        offset_t PROXY_2_MAX_HOST_TAG_OFFSET = NO_VALUE;
        offset_t PROXY_2_MIN_HOST_TAG_OFFSET = NO_VALUE;
        offset_t PROXY_2_MAX_COUNTER_HOST_TAG_OFFSET = NO_VALUE;
        if (IS_SET(fields_mask, 7)) {
            PROXY_2_MAX_HOST_TAG_OFFSET = proxy_2_pos;
            proxy_2_pos += Int::SIZEOF;
        }
        if (IS_SET(fields_mask, 8)) {
            PROXY_2_MIN_HOST_TAG_OFFSET = proxy_2_pos;
            proxy_2_pos += Int::SIZEOF;
        }
        if (IS_SET(fields_mask, 9)) {
            PROXY_2_MAX_COUNTER_HOST_TAG_OFFSET = proxy_2_pos;
            proxy_2_pos += Int::SIZEOF;
        }
        Proxy proxy_2 = Proxy::fetch(stream, PROXY_2_SIZEOF);
        MultiValue_BASE result(std::move(PROXY_1_SIZEOF),
                               std::move(PROXY_1_COUNTER_OFFSET),
                               std::move(PROXY_1_COUNTER_EQ_1_OFFSET),
                               std::move(PROXY_1_VALUE_SET_OFFSET),
                               std::move(PROXY_1_VALUE_MIN_OFFSET),
                               std::move(PROXY_1_VALUE_MAX_OFFSET),
                               std::move(PROXY_1_VALUE_SUM_OFFSET),
                               std::move(PROXY_1_VALUE_SQUARE_OFFSET),
                               std::move(proxy_1),
                               std::move(uniques),
                               std::move(centroids),
                               std::move(PROXY_2_SIZEOF),
                               std::move(PROXY_2_MAX_HOST_TAG_OFFSET),
                               std::move(PROXY_2_MIN_HOST_TAG_OFFSET),
                               std::move(PROXY_2_MAX_COUNTER_HOST_TAG_OFFSET),
                               std::move(proxy_2));
...
private:
    MultiValue_BASE(offset_t&& PROXY_1_SIZEOF,
                    offset_t&& PROXY_1_COUNTER_OFFSET,
                    offset_t&& PROXY_1_COUNTER_EQ_1_OFFSET,
                    offset_t&& PROXY_1_VALUE_SET_OFFSET,
                    offset_t&& PROXY_1_VALUE_MIN_OFFSET,
                    offset_t&& PROXY_1_VALUE_MAX_OFFSET,
                    offset_t&& PROXY_1_VALUE_SUM_OFFSET,
                    offset_t&& PROXY_1_VALUE_SQUARE_OFFSET,
                    Proxy&& proxy_1,
                    std::optional<string>&& uniques,
                    std::optional<array<centroid>>&& centroids,
                    offset_t&& PROXY_2_SIZEOF,
                    offset_t&& PROXY_2_MAX_HOST_TAG_OFFSET,
                    offset_t&& PROXY_2_MIN_HOST_TAG_OFFSET,
                    offset_t&& PROXY_2_MAX_COUNTER_HOST_TAG_OFFSET,
                    Proxy&& proxy_2) noexcept
        : PROXY_1_SIZEOF(std::move(PROXY_1_SIZEOF))
        , PROXY_1_COUNTER_OFFSET(std::move(PROXY_1_COUNTER_OFFSET))
        , PROXY_1_COUNTER_EQ_1_OFFSET(std::move(PROXY_1_COUNTER_EQ_1_OFFSET))
        , PROXY_1_VALUE_SET_OFFSET(std::move(PROXY_1_VALUE_SET_OFFSET))
        , PROXY_1_VALUE_MIN_OFFSET(std::move(PROXY_1_VALUE_MIN_OFFSET))
        , PROXY_1_VALUE_MAX_OFFSET(std::move(PROXY_1_VALUE_MAX_OFFSET))
        , PROXY_1_VALUE_SUM_OFFSET(std::move(PROXY_1_VALUE_SUM_OFFSET))
        , PROXY_1_VALUE_SQUARE_OFFSET(std::move(PROXY_1_VALUE_SQUARE_OFFSET))
        , m_proxy_1(std::move(proxy_1))
        , m_uniques(std::move(uniques))
        , m_centroids(std::move(centroids))
        , PROXY_2_SIZEOF(std::move(PROXY_2_SIZEOF))
        , PROXY_2_MAX_HOST_TAG_OFFSET(std::move(PROXY_2_MAX_HOST_TAG_OFFSET))
        , PROXY_2_MIN_HOST_TAG_OFFSET(std::move(PROXY_2_MIN_HOST_TAG_OFFSET))
        , PROXY_2_MAX_COUNTER_HOST_TAG_OFFSET(std::move(PROXY_2_MAX_COUNTER_HOST_TAG_OFFSET))

...
#define COMPILE_TIME_CRC32_STR(x) (utils::crc32<sizeof(x) - 2>(x) ^ 0xFFFFFFFF)


template <uint32_t MAX_VALUE>
uint32_t random_mask(std::default_random_engine& engine)
{
    static std::uniform_int_distribution<uint32_t> ud(0, MAX_VALUE);
    return ud(engine);
}
...
{
    return (crc32<idx - 1>(str) >> 8) ^ crc32_tab[(crc32<idx - 1>(str) ^ str[idx]) & 0x000000FF];
}
